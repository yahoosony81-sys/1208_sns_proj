...
2. 프로젝트 선택
3. **Integrations** 메뉴로 이동 (또는 [Supabase 통합 설정 페이지](https://dashboard.clerk.com/setup/supabase)로 직접 이동)
4. **Supabase** 통합 찾기
...
**3-2. Supabase Dashboard에서 Clerk 인증 제공자 추가**
...
5. **"Add provider"** 클릭
6. 제공자 목록에서 **"Clerk"** 선택
7. 3-1에서 복사한 **Clerk domain** 입력 (예: `your-app-12.clerk.accounts.dev`)
8. **"Save"** 또는 **"Add Provider"** 클릭

**3-3. 통합 확인**

- [상세 통합 가이드 문서](./docs/clerk-supabase-integration.md) 참고
...
2. 다음 값들을 복사하여 `.env` 파일에 입력:
   ...
   # 또는 공식 가이드에 맞춰 PUBLISHABLE_KEY 사용 가능
   # NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY="<anon public key>"
   SUPABASE_SERVICE_ROLE_KEY="<service_role secret key>"
   NEXT_PUBLIC_STORAGE_BUCKET="uploads"
   ```
   
   > **참고**: Supabase 공식 가이드에서는 `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`를 사용하지만, 
   > 프로젝트는 `NEXT_PUBLIC_SUPABASE_ANON_KEY`와 `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` 둘 다 지원합니다.
...
- **[Supabase Next.js 퀵스타트](./docs/supabase-quickstart.md)** - Supabase 공식 가이드 기반 연결 방법
- **[Clerk + Supabase 통합 가이드](./docs/clerk-supabase-integration.md)** - 최신 통합 방법 및 설정 가이드
- **[Clerk 한국어 로컬라이제이션](./docs/clerk-localization.md)** - Clerk 컴포넌트 한국어 설정 가이드
- **[RLS 정책 예시](./docs/rls-policies-examples.md)** - Row Level Security 정책 작성 예시 및 모범 사례

## 추가 설정 및 팁

### Clerk 한국어 설정

프로젝트에 이미 Clerk 한국어 로컬라이제이션이 적용되어 있습니다. `app/layout.tsx`의 `ClerkProvider`에서 한국어 로컬라이제이션이 설정되어 있으며, `lib/clerk/localization.ts`에서 관리됩니다.
...
추가 테이블 생성 시 RLS 정책을 반드시 설정하세요. 자세한 예시는 [RLS 정책 예시 문서](./docs/rls-policies-examples.md)를 참고하세요.
